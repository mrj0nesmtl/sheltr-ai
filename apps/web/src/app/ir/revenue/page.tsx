...
export default function IRRevenuePage() {
  ...
  useEffect(() => {
    if (!authLoading && !user) {
      router.push('/login?redirect=/ir/revenue');
    }
    ...
    const loadRevenueData = async () => {
      try {
        ...
        if (revenueSnap.exists()) {
          ...
          setRevenueData(data);
          console.log('✅ Revenue data loaded from Firestore for IR');
        } else {
          setError('Revenue data not found.');
        }
      } catch (err) {
  ...
  const getCategoryMonthlyBreakdown = (monthIndex: number) => {
    if (!revenueData) return { core: 0, enterprise: 0, defi: 0 };
    
    const core = revenueData.streams.core_revenue.streams.reduce((sum, stream) => {
      return sum + (stream.budget_values[monthIndex] || 0);
    }, 0);
    
    const enterprise = revenueData.streams.enterprise_revenue.streams.reduce((sum, stream) => {
      return sum + (stream.budget_values[monthIndex] || 0);
    }, 0);
    
    const defi = revenueData.streams.defi_revenue.streams.reduce((sum, stream) => {
      return sum + (stream.budget_values[monthIndex] || 0);
    }, 0);
  ...
  // Use revenueData directly (no edit mode for IR)
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-3 mb-2">
                <h1 className="text-3xl font-bold">{revenueData.title}</h1>
              ...
              <p className="text-muted-foreground">
                Revenue Projections • Sep 2025 - Aug 2027 • {revenueData.period.fiscal_years}
              </p>
            </div>
          ...
          <Card className="border-2 border-green-200 dark:border-green-900">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-green-600 dark:text-green-400">
                {formatCurrency(revenueData.targets.year_1_target)}
              </div>
          ...
          <Card className="border-2 border-blue-200 dark:border-blue-900">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-blue-600 dark:text-blue-400">
                {formatCurrency(revenueData.targets.year_2_target)}
              </div>
          ...
          <Card className="border-2 border-purple-200 dark:border-purple-900">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-purple-600 dark:text-purple-400">
                {formatCurrency(revenueData.targets.total_24_month)}
              </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mt-1">{revenueData.streams.core_revenue.streams.length} streams</p>
              </div>
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mt-1">{revenueData.streams.enterprise_revenue.streams.length} streams</p>
              </div>
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mt-1">{revenueData.streams.defi_revenue.streams.length} streams</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <AccordionItem value="revenue-chart" className="border rounded-lg px-6 bg-white dark:bg-slate-900">
            ...
            <AccordionContent className="pb-6">
              <div className="space-y-4">
                {revenueData.period.months.map((month, index) => {
                  ...
                  const totalRevenue = revenueData.calculated.budget_monthly_revenue[index];
                  
                  const maxRevenue = isFiltered 
                    ? Math.max(...revenueData.period.months.map((_, idx) => getCategoryMonthlyBreakdown(idx)[selectedCategory as keyof typeof breakdown]))
                    : Math.max(...revenueData.calculated.budget_monthly_revenue);
