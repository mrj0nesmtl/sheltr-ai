...
OUTPUT_FILE="$PROJECT_ROOT/docs/02-architecture/PROJECT-TREE.md"

# Verify we're in the right directory
if [[ ! -f "$PROJECT_ROOT/package.json" && ! -f "$PROJECT_ROOT/apps/web/package.json" ]]; then
    echo -e "${RED}❌ Error: Not in SHELTR project root directory${NC}"
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    # macOS
    sed -i '' "s/\$(date)/$CURRENT_DATE/" "$OUTPUT_FILE"
    sed -i '' "s/\$(git log -1 --format=\"%cd\" --date=short)/$LAST_COMMIT_DATE/" "$OUTPUT_FILE"
else
    # Linux
    sed -i "s/\$(date)/$CURRENT_DATE/" "$OUTPUT_FILE"
    sed -i "s/\$(git log -1 --format=\"%cd\" --date=short)/$LAST_COMMIT_DATE/" "$OUTPUT_FILE"
...
echo -e "${GREEN}📊 Generated on: $CURRENT_DATE${NC}"
echo -e "${GREEN}📅 Last commit: $LAST_COMMIT_DATE${NC}"
